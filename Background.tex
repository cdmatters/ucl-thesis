...
This $p(\{s_i\})$ term in this equation represents the probability of the sequence occuring naturally in the language. This term is what language models aim to represent.
...
In our work, language models are implicitly used in our sequence generation, where we seek to generate likely natural language given our code features $\mathcal{C}$, and we draw inspiration from them for the architectures of our models.

\subsection{Language Generation}

A traditional approach to conditionally generating language, is that of machine translation.
In machine translation, we seek to generate the most likely sequence of a target language, $\{r_1, r_2,..., r_m\}$, given an sequence in an origin language  $\{s_1, s_2,..., s_n\}$. 
In this case, we ideally maximise the probability:

$$ p(r_1, r_2,..., r_m| s_1, s_2,...,s_n )$$


Although the historic contexts of the machine translation revolve around two sequences of words in differnet languages, in principle such an approach extends to other settings.
...
\subsection{Regularization \& Training Methods} % (fold)
