...
\makebox[\linewidth][c]{
    ...
    Almost have the datapoints have a unique name + description, indicating a variety of descriptions for the same name (\textit{left}). However \mintinline[]{python}{tensorflow}'s \mintinline[]{python}{name}: \textit{``A name for operation (optional).''} still accounts for 3.3\% of the collected data.}
