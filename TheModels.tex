...
We defined our optimisation using
...
\subsubsection{Decoder, Optimiser}
...
In this model we consider the two encoders above as operators that return tuples. We simply return the concatenation of each component of tuple:
\begin{align}
    s^{s2s}  \gets \mathcal{SEQ}(\mathbf{x}) \\
    s^{c2v} \gets \mathcal{COD}(\mathbf{x}) \\
    \mathbf{s}_1^T = [ (\mathbf{s}^{s2s}_1)^T , (\mathbf{s}^{c2v}_1)^T ] \\
    \mathbf{s}_2^T = [ (\mathbf{s}^{s2s}_2)^T , (\mathbf{s}^{c2v}_2)^T ] \\
    s = (\mathbf{s_1}, \mathbf{s_2})
\end{align}
where $\mathcal{SEQ}$ indicates the encoder component of the Seq2Seq model and the  $\mathcal{COD}$ indicates the component of the Code2Vec Decoder, and $s^\mathcal{X}$ indicates the a tuple of vectors from encoder operation $\mathcal{X}$.


\subsubsection{Decoder, Attention, Dropout, Optimiser}
