...
In our first examination of the computer readable channel we were keen to validate our initial hypothesis that simply the code could be informative when predicting argument descriptions.
...
Since this experiment would look exclusively at code paths, ignoring the variable name, we used the Full Random-Split Dataset in this investigation.
We extracted the syntax trees from the training source code and extracted a vocabulary of paths and terminal nodes from the training set. 
The vocabulary of the paths and terminal nodes was set to 15000 each and the limit of code paths per point limited to 5000, for all models. 
The rest of the tokenization procedure is exactly as outlined in section X. 
...
These chose a description from a set of training samples by:
    ...
    \end{itemize}
Each of these was run 50 times to generate an estimate of the variance based on the stochasticity of the Rote Learner.
We then ran the Code2Vec Decoder, following the usual training procedure, for a maximum of 100 epochs, and selecting the model with the best BLEU score.
The hyperparameters for this model are found in Table {X}.

We report that the Rote Learner benefits most from taking the hardest attitude possible to matching full code paths, with  Full Path Max criterion performing best.
This provided a strong baseline BLEU score of $ 11.82 \pm  0.16 $ and  $ 12.61 \pm 0.12 $  on validtion and test respectively. 
The next strongest criterion was then the Sub Path Max criterion which achieved a BLEU score about 5 points worse, of   $ 6.93 \pm  0.34 $ and $ 7.09 \pm 0.22 $ on validation and test respectively.  
The two remaining criteria which chose randomly from lists of training examples populated proportionally to the number of matches with the set of test paths, performed significantly worse again, losing between six and seven BLEU points compared to their counterparts. 
...
The performance of the Code2Vec Decoder then reiterated this assumption by significantly outperforming the strongest Rote Learner, with a BLEU score of 
$ 18.94 $ and $ 18.13 $ on validation and test. 
From these results we concluded that both a neural approach could outperform Rote Learner and learn significant features from the code in the dataset.
