...
In these we see very little overlap, though the translations make grammatical sense. In particular the types of descriptions generated appear shorter and more general than their counterparts with the Rote Learner.
...
Similarly with code paths, we found that a lot of information is lost by splitting data according to library. This was particularly pronounced because, unlike with the descriptions, the vocabulary for code paths is set entirely by the training set.
...
% Then CODE investigate. 



% LIB SPLIT
% (3697, 121)
% UNK Tokens 5.358638863079008
% UNIQUE (2866,)
% (23463, 121)
% UNK Tokens 5.4953593054582
% UNIQUE (4701,)
% Vocab of Valid not in Train 453
% Vocab of Train not in Vocab?
% Vocab of Test not in vocab?

% RANDOM SPLIT
% (3453, 121)
% UNK Tokens 5.279460246875718
% UNIQUE (3012,)
% (24197, 121)
% UNK Tokens 5.247896012242944
% UNIQUE (5249,)
% Vocab of Valid not in Train 122



%  Libraries often treat code objects conceptually, having a unified framework to deal 

%  In this (relatively) small dataset, the training language can end up being very context specific, resulting in a failure of the model being able to generalise. T

%  mean that the models are fundamentally  different language mig. Since the target descriptions are real world argument descriptions, with all the context of the libraries they were in, it is very hard fro the model generate a large ngram overlap with the target descriptions.

% For one thing the model already is limited by its access to the vocabulary. In the RandomSplit library the vo
% * UNK tokens in valid \& main
% * UNK paths train and test

% Furthermore 


% * qualitative looking at desscriptions
% * one word overlap 
% * also challenge of the dataset


\textbf{Argument}: \mintinline[]{python}{kind$<$END$>$}\\
...
\textbf{Argument}: \mintinline[]{python}{bucket$<$END$>$}\\
\textbf{D}: bucket to which the notification is bound .\\
\textbf{P}: list of $<$UNK$>$ to be used .\\
\\

\textbf{Argument}: \mintinline[]{python}{key$<$END$>$}\\
...
\textbf{Argument}: \mintinline[]{python}{database$<$END$>$}\\
\textbf{D}: database to use\\
\textbf{P}: the $<$UNK$>$ to use for the $<$UNK$>$ .\\
\\

\textbf{Argument}: \mintinline[]{python}{is\_training$<$END$>$}\\
\textbf{D}: bool . is it training phase or not ?\\
\textbf{P}: if ` true ` , the $<$UNK$>$ is used to the $<$UNK$>$ .\\
\\

\textbf{Argument}: \mintinline[]{python}{weight$<$END$>$}\\
...
\textbf{Argument}: \mintinline[]{python}{note\_sequence$<$END$>$}\\
\textbf{D}: a music\_pb2.notesequence proto .\\
\textbf{P}: a ` $<$UNK$>$ ` object .\\
\\

\textbf{Argument}: \mintinline[]{python}{x$<$END$>$}\\
\textbf{D}: the x coordinates of the nodes defining the curves .\\
\textbf{P}: the number of jobs to use .\\
\\

\textbf{Argument}: \mintinline[]{python}{reference\_beats$<$END$>$}\\
\textbf{D}: reference beat times , in seconds\\
\textbf{P}: the input array .\\
\\

\textbf{Argument}: \mintinline[]{python}{file\_like$<$END$>$}\\
