...
We ran our Rote Model as described in section \ref{sec:general_procedure}. We then ran our on the basic Seq2Seq architectures in an reverse ablation study: we started from a basic seq-to-seq, then adding attention, a bidirectional encoder and finally dropout.
...
This is surpassed by the bidirectional Seq2Seq with attention and dropout,  which achieves a BLEU-4 score of $12.46 $ and $12.72$ on validation and test set respectively.
In our ablation study we note the improvement that each new feature adds, noting that the pure Seq2Seq model underperforms the Rote Learner, scoring $7.76 $ and $  7.63$ on the validation and test set. 
